...
func (s *Server) handleRegister(c *gin.Context) {
	...
	// NOTE: Orphan record adoption is intentionally not supported. Otherwise a
	// newly registered user after an account reset could inherit a previous
	// owner's API credentials and wallet keys.
...
// dummyPasswordHash is compared when an email does not exist so login timing
// stays roughly uniform and does not reveal whether an account exists.
...
// Password/account recovery is intentionally not exposed over HTTP. Use the
// local CLI (`nofx reset-password` / `nofx reset-account`) from the host.
