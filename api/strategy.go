...
func strategyUsesNofxOSData(config *store.StrategyConfig) bool {
	return config.Indicators.EnableQuantData ||
		config.Indicators.EnableOIRanking ||
...
func validateStrategyConfig(config *store.StrategyConfig) []string {
	...
	if strategyUsesNofxOSData(config) && config.Indicators.NofxOSAPIKey == "" {
		warnings = append(warnings, "Quant data uses the self-hosted DATA_GATEWAY_URL exchange-public data service. Ensure the data gateway is running before starting this strategy.")
	}
