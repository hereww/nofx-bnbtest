...
- 风险回报比 ≥ 1:3
- 最多持仓 3 个币种
- 杠杆限制：遵守策略配置上限；交易所交易对和名义仓位档位允许时最高 125x
- 保证金使用率 ≤ 90%
...
#### 4. 杠杆限制
**要求**:
- 山寨币: 遵守策略配置的最大杠杆；交易所档位允许时最高 125x
- BTC/ETH: 遵守策略配置的最大杠杆；交易所档位允许时最高 125x
...
杠杆使用：
- 山寨币: 使用策略配置的杠杆；稳健策略通常显著低于交易所上限
- BTC/ETH: 使用策略配置的杠杆；高杠杆只应在明确配置且风控充分时使用
...
4. 杠杆: 遵守策略配置上限；交易所交易对和名义仓位档位允许时最高 125x
5. 保证金使用率 ≤ 90%
