...
export const indicator = {
  ...
  netflowRankingDesc: { zh: '交易所公开数据资金压力近似', en: 'Exchange-public fund pressure estimate', es: 'Estimacion de presion de fondos' },
  netflowRankingNote: { zh: '基于 OI、成交额、价格和资金费率近似资金压力，不等同于真实链上流入流出', en: 'Estimates fund pressure from OI, volume, price, and funding; not real on-chain inflow/outflow', es: 'Estimacion basada en OI, volumen, precio y funding' },
  ...
  nofxosTitle: { zh: '自研量化数据网关', en: 'Self-hosted Quant Data Gateway', es: 'Gateway quant autohospedado' },
  nofxosDesc: { zh: '交易所公开数据驱动的本地兼容数据服务', en: 'Exchange-public local compatible data service', es: 'Servicio local compatible con datos publicos de exchanges' },
  ...
  apiKeyPlaceholder: { zh: '通常留空：由 DATA_GATEWAY_URL 提供数据', en: 'Usually empty: data comes from DATA_GATEWAY_URL', es: 'Normalmente vacio: datos desde DATA_GATEWAY_URL' },
  connected: { zh: '网关模式', en: 'Gateway Mode', es: 'Modo gateway' },
  notConfigured: { zh: '网关模式', en: 'Gateway Mode', es: 'Modo gateway' },
  nofxosDataSources: { zh: '数据网关数据源', en: 'Gateway Data Sources', es: 'Fuentes del gateway' },
  configureApiKey: { zh: '数据源由自研 DATA_GATEWAY_URL 服务提供，请确认网关已启动', en: 'Data is provided by the self-hosted DATA_GATEWAY_URL service. Make sure the gateway is running.', es: 'Los datos los proporciona el servicio DATA_GATEWAY_URL autohospedado.' },
  dataGatewayRoutingHint: { zh: '策略会读取自研数据网关；这里的旧 Key 字段通常留空。', en: 'Strategies read the self-hosted data gateway; leave this legacy key field empty in normal use.', es: 'Las estrategias leen el gateway autohospedado. Normalmente deje este campo vacio.' },
};
